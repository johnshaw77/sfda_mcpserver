...
export class GetSalaryInfoTool extends BaseTool {
  ...
  async _simulateSalaryQuery(
    ...
    // 檢查員工是否存在
    if (!this._isValidEmployee(employeeNo)) {
      throw new Error(`員工編號 ${employeeNo} 不存在`);
    }
   ...
   * 檢查員工是否存在
   */
  _isValidEmployee(employeeNo) {
    const validEmployees = ["A123456", "A123457", "A123458", "A123459"];
    return validEmployees.includes(employeeNo);
  }
