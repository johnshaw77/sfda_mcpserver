...
class Logger {
  ...
  // 兼容舊版 Winston logger 的方法
  async verbose(message, meta = {}) {
    return this.trace(message, meta);
  }
  ...
  async logToolCall(logData) {
    ...
    return this.info(`工具調用: ${toolName} (${status})`, {
      category: "tool-call",
      toolName,
      executionId,
      status,
      ...
      resultSize,
      error,
      clientId: context.userId || context.sessionId,
      meta: context,
    });
  }
...
// 為了向後兼容，添加與舊版 Winston logger 相同的方法
