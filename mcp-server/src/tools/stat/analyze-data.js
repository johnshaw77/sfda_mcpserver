...
export const analyzeData = {
  name: "analyze_data",
  description: "智能分析 CSV 數據結構，建議適合的統計檢定方法",
  inputSchema: {
    type: "object",
    properties: {
      csvData: {
...
export async function handleAnalyzeData(args) {
  try {
    ...
    if (!args.csvData || args.csvData.trim().length === 0) {
      throw new Error("CSV 數據不能為空");
    }

    // 分析數據結構
    const dataStructure = await statService.analyzeDataStructure(args.csvData);
    ...
    // 生成分析報告
    const report = generateAnalysisReport(dataStructure, suggestions, args);
 ...
 */
function generateAnalysisReport(dataStructure, suggestions, args) {
  let report = "";
  ...
  if (suggestions.suggestions && suggestions.suggestions.length > 0) {
    ...
    suggestions.suggestions.forEach((suggestion, index) => {
      ...
      const emoji = getConfidenceEmoji(suggestion.confidence);

      report += `### ${index + 1}. ${getTestDisplayName(suggestion.test)} ${emoji}\n\n`;
      ...
      if (suggestion.test === "ttest") {
        report += generateTTestGuidance(suggestion, dataStructure);
      } else if (suggestion.test === "chisquare") {
        report += generateChiSquareGuidance(suggestion, dataStructure);
      } else if (suggestion.test === "anova") {
        report += generateANOVAGuidance(suggestion, dataStructure);
      }
    ...
    if (suggestions.recommendation) {
      ...
      report += "## 🌟 最佳建議\n\n";
      report += `基於數據特徵，**${getTestDisplayName(best.test)}** 是最適合的分析方法。\n\n`;
  ...
  const qualityChecks = performQualityChecks(dataStructure);
 ...
 * @returns {string} 使用指導
 */
function generateTTestGuidance(suggestion, dataStructure) {
  let guidance = "**使用指導**:\n";
 ...
 * @returns {string} 使用指導
 */
function generateChiSquareGuidance(suggestion, dataStructure) {
 ...
 * @returns {string} 使用指導
 */
function generateANOVAGuidance(suggestion, dataStructure) {
 ...
 * @returns {Array} 檢查建議
 */
function performQualityChecks(dataStructure) {
  const checks = [];
