...
export class PerformANOVATool extends BaseTool {
  ...
  generateANOVAReport(result, args) {
    ...
    // 情境化解釋
    report += this.generateContextualInterpretation(result, args, isSignificant);
  ...
  generateAssumptionChecks(result, args) {
    ...
    if (result.normality_tests) {
      ...
      result.normality_tests.forEach((test, i) => {
        const groupName = args.context?.variables?.group_names?.[i] || `組別 ${i + 1}`;
  ...
  generateRecommendations(result, args, isSignificant) {
    ...
    if (isSignificant) {
      recommendations += "- 結果顯示組間存在顯著差異，建議進行事後檢定 (post-hoc tests)\n";
      recommendations += "- 可考慮使用 Tukey HSD、Bonferroni 或 Scheffé 檢定找出具體差異\n";
    ...
    // 樣本大小建議
    const totalSampleSize = args.data.groups.reduce((sum, group) => sum + group.length, 0);
    ...
    if (result.normality_tests && result.normality_tests.some(test => test.p_value <= 0.05)) {
      recommendations += "- 資料不符合常態分佈，考慮使用 Kruskal-Wallis 非參數檢定\n";
    }
