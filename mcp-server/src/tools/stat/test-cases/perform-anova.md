...
### 情境 3: 軟板製造良率分析 - 生產線比較
**研究背景**: 比較四條軟板生產線的良率表現是否有顯著差異
**研究設計**: 完全隨機設計，每條生產線收集12批次良率數據
**假設**: H₀: μ₁ = μ₂ = μ₃ = μ₄ vs H₁: 至少有一組不等
...
### 情境 4: 軟板製造工藝段良率分析
**研究背景**: 比較軟板製造五個關鍵工藝段的良率差異
**研究設計**: 每個工藝段收集10批次良率數據
**假設**: H₀: μ₁ = μ₂ = μ₃ = μ₄ = μ₅ vs H₁: 至少有一組不等
...
### 範例 3: 軟板生產線良率比較 - 白話提問
**用戶提問**: "我們廠有四條軟板生產線，老闆想知道哪條線的良率比較好，是不是每條線都差不多。我收集了各條線最近的良率數據：

第一條線：94.2, 95.8, 94.7, 96.3, 95.1, 95.9, 94.5, 96.1, 95.4, 94.8, 95.7, 94.9
第二條線：92.1, 93.7, 92.6, 94.2, 93.0, 93.8, 92.4, 94.0, 93.3, 92.7, 93.6, 92.8
第三條線：96.8, 97.4, 96.2, 98.1, 97.0, 97.6, 96.5, 97.9, 97.3, 96.9, 97.5, 96.7
第四條線：89.5, 91.2, 90.1, 92.8, 90.7, 91.9, 89.8, 92.4, 91.5, 90.3, 91.7, 90.6

能幫我分析看看這四條線的良率是不是真的有差別？還是只是正常的波動？"

**AI 回應**: 您想比較四條生產線的良率是否有顯著差異，這需要用 ANOVA 分析。ANOVA 可以同時比較多組數據，告訴我們這些差異是統計上顯著的，還是只是正常的隨機波動。
...
```json
{
  ...
  "context": {
    ...
    "group_names": ["第一條線", "第二條線", "第三條線", "第四條線"],
    "factor_name": "生產線"
  }
}
```
...
### 範例 4: 軟板工藝段分析 - 白話提問
**用戶提問**: "我們軟板製程有好幾個步驟，我想看看哪個步驟最容易出問題。我統計了各個工藝段的合格率：

基材處理：98.5, 99.1, 98.8, 99.4, 98.9, 99.2, 98.7, 99.3, 99.0, 98.6
圖形轉移：95.2, 96.8, 95.7, 97.3, 96.1, 96.9, 95.5, 97.1, 96.4, 95.8
蝕刻製程：91.8, 93.5, 92.3, 94.1, 92.7, 93.8, 92.1, 93.9, 93.2, 92.5
表面處理：97.1, 98.2, 97.6, 98.7, 97.9, 98.4, 97.4, 98.6, 98.1, 97.7
最終檢測：88.6, 90.2, 89.1, 91.5, 89.7, 90.8, 88.9, 91.1, 90.4, 89.3

這些工藝段的良率差異大嗎？哪個步驟需要重點改善？"

**AI 回應**: 您想比較五個工藝段的良率差異，找出需要改善的製程瓶頸。用 ANOVA 分析可以告訴我們這些工藝段之間是否有顯著差異，以及哪些步驟的表現明顯不同。
